...
void si5351_set_correction(int32_t correction)
{
...
void si5351_Calc(int32_t Fclk, si5351PLLConfig_t* pll_conf, si5351OutputConfig_t* out_conf) {
    ...
    Fclk = Fclk - (int32_t)((((double)Fclk)/10000000.0)*((double)si5351Correction));
...
void si5351_SetupCLK(uint8_t ch, int32_t Fclk, si5351DriveStrength_t driveStrength, uint8_t phase)
{
	...
	if(ch == 0){
		...
		si5351_SetupOutput(0, SI5351_PLL_A, driveStrength, &out_conf, phase);
	} else {
		si5351_SetupPLL(SI5351_PLL_B, &pll_conf);
		si5351_SetupOutput(ch, SI5351_PLL_B, driveStrength, &out_conf, 0);
	}
}
